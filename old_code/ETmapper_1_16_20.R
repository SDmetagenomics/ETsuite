    ...
    for (i in 1:nrow(batch_file)){
      ...
      if(paired_end_data == TRUE){
       ...
       if(paired_end_data == FALSE){
         jm_workflow_stats[i,4] <- as.numeric(system(paste0("grep 'Total reads processed:' ",out_dir,"/logs/",batch_file[i,1],".trim.log ","| awk '{print $4}' | sed 's/,//g'"), intern = T)) # Total Reads
         jm_workflow_stats[i,5] <- as.numeric(system(paste0("grep 'Reads with adapters:' ",out_dir,"/logs/",batch_file[i,1],".trim.log ","| awk '{print $4}' | sed 's/,//g'"), intern = T)) # R1_adap
         jm_workflow_stats[i,7] <- NA # R2_adap
         jm_workflow_stats[i,9] <- count.primer() #Tn_Primer
         jm_workflow_stats[i,11] <- as.numeric(system(paste0("grep 'Reads with adapters:' ",out_dir,"/logs/",batch_file[i,1],".clean.log ","| awk '{print $4}' | sed 's/,//g'"), intern = T)) #Model_Keep
         jm_workflow_stats[i,13] <- NA # R2_model
         jm_workflow_stats[i,15] <- as.numeric(system(paste0("grep 'Reads written' ",out_dir,"/logs/",batch_file[i,1],".clean.log ","| awk '{print $5}' | sed 's/,//g'"), intern = T)) #Good_Keep
         jm_workflow_stats[i,17] <- as.numeric(system(paste0("sed 1d ",out_dir,"/hits/",batch_file[i,1],".hits | wc -l"), intern = T)) #Raw_map 
       }
...
if (wf == "jm"){
  ...
  if(paired_end_data == TRUE){
    ...
    bc_flank <- system(paste0("grep -o \"NN.*.NN\" ",md," | sed 's/N//g'"), intern = T) ## Needs to be modified to: "grep -o "^NN.*.NN" models.fa | sed 's/N//g'
