...
etstats_version <- c("v0.03")
wf <- "ss"
dat_in <- "~/Dropbox/Banfield_Lab_Files/Projects/mCAFE/mCAFE_Project/Studies/20_1_21_BR_35_ET/ET_mapper/"
...
check.env <- function(){
  ...
  if (wf == "ss"){
    ...
    if (jm_exist == TRUE){
      ...
      env_summary$jm_pe_map <- system(paste0("grep 'Paired End' ",dat_in,"jm/run_log.txt | awk '{print $3}'"), intern = T)
      cat(
    ...
    if (lm_exist == TRUE){
      ...
      env_summary$lm_pe_map <- system(paste0("grep 'Paired End' ",dat_in,"lm/run_log.txt | awk '{print $3}'"), intern = T)
      cat(
